...
Problem Client::newProblem(const std::string& name, long seed) {
    ...
    std::string url = endpoint + "/interview/challenge/new?auth=" + auth + "&name=" + name + "&seed=" + std::to_string(seed);
    ...
    std::cout << "Fetched new test problem, id=" << id << ": " << url << std::endl;
...
std::string Client::solve(const std::string& testId, std::chrono::microseconds rate, std::chrono::microseconds min, std::chrono::microseconds max, const std::vector<Action>& actions) {
    ...
    auto response = cpr::Post(cpr::Url{url}, cpr::Header{{"Content-Type", "application/json"}, {"x-test-id", testId}}, cpr::Body{solution.dump()});
    if (response.status_code != 200) {
        throw std::runtime_error(url + ": " + response.status_line);
    }
